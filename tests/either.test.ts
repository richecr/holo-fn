...
describe("Either", () => {
	...
	it("Right.validate should keep value when predicate returns true", () => {
		const result = new Right<string, number>(25).validate(
			(x) => x >= 18,
			"Must be 18+",
		);
	...
	it("Right.validate should convert to Left when predicate returns false", () => {
		const result = new Right<string, number>(15).validate(
			(x) => x >= 18,
			"Must be 18+",
		);
		expect(result.isLeft()).toBe(true);
		expect(result.match({ left: (e) => e, right: (_) => "" })).toBe(
			"Must be 18+",
		);
	});
	...
	it("Right.validate should support chaining multiple validations", () => {
		...
		expect(validateAge(15).match({ left: (e) => e, right: (_) => "" })).toBe(
			"Must be 18+",
		);
		expect(validateAge(-5).match({ left: (e) => e, right: (_) => "" })).toBe(
			"Age cannot be negative",
		);
		expect(validateAge(200).match({ left: (e) => e, right: (_) => "" })).toBe(
			"Age too high",
		);
	});
...
describe("Either - Curried Helpers", () => {
	...
	it("should apply curried mapLeft function to Left", () => {
		...
		expect(result.match({ left: (e) => e, right: () => "" })).toBe(
			"Mapped Error: error",
		);
	});
