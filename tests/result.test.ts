...
describe("Result", () => {
	...
	it("Ok.validate should support chaining multiple validations", () => {
		...
		expect(validateAge(15).match({ ok: (_) => "", err: (e) => e })).toBe(
			"Must be 18+",
		);
		expect(validateAge(-5).match({ ok: (_) => "", err: (e) => e })).toBe(
			"Age cannot be negative",
		);
		expect(validateAge(200).match({ ok: (_) => "", err: (e) => e })).toBe(
			"Age too high",
		);
	});
...
describe("Result - Curried Helpers", () => {
	...
	it("should apply curried mapErr function to Err", () => {
		...
		expect(result.match({ ok: (_) => "", err: (e) => e })).toBe(
			"Mapped Error: initial error",
		);
	});
	...
	it("all should return Err with all errors when any are Err", () => {
		...
		const result = all(results);
		expect(result.isErr()).toBe(true);
		expect(result.match({ ok: (_) => [], err: (e) => e })).toEqual([
			"error1",
			"error2",
		]);
	});
	...
	it("sequence should stop at first error unlike all", () => {
		const results = [
			err<number, string>("First error"),
			err<number, string>("Second error"),
			ok<number, string>(1),
		];
		const result = sequence(results);
	...
	it("partition should return all errs when all fail", () => {
		const results = [
			err<number, string>("e1"),
			err<number, string>("e2"),
			err<number, string>("e3"),
		];
		const { oks, errs } = partition(results);
		expect(oks).toEqual([]);
